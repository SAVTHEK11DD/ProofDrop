(function () {
  ...
  function setWalletStatus(message, kind) {
    document.querySelectorAll("#walletStatus").forEach((el) => {
      const compactWallet = el.closest("[data-compact-wallet]");
      const connected = kind === "success" && walletAddress;
      el.textContent = connected ? shortAddress(walletAddress) : "";
      if (!compactWallet && !connected) el.textContent = message;
  ...
  function updateWalletButtons() {
    document.querySelectorAll("#connectWalletBtn").forEach((button) => {
      button.innerHTML =
        WALLET_ICON + (walletAddress ? "Disconnect wallet" : "Connect wallet");
